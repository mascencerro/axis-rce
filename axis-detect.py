...
# Original Python2 source: https://github.com/mcw0/PoC/blob/master/axis-detect.py
# "Anonymously detect Model and Firmware of Axis devices"
# [SOF]
...
class HTTPconnect:
	...
	def Send(self, uri):
		...
		url = f"{self.proto}://{self.host}{uri}"

		if self.verbose:
			print(f"[Verbose] Sending: {url}")
		...
		if self.credentials:
			...
			if self.verbose:
				print(f"[Verbose] User: {Basic_Auth[0]} Password: {Basic_Auth[1]}")
			try:
				pwd_mgr = request.HTTPPasswordMgrWithDefaultRealm()
				pwd_mgr.add_password(None, url, Basic_Auth[0], Basic_Auth[1])
				auth_handler = request.HTTPBasicAuthHandler(pwd_mgr)
		...
		for tmp in range(0,len(URI)):
			try:
				if Vendor and Product:
					print(f"[>] Trying to detect firmware version...({int(tmp+1)})")
				else:
					print(f"[>] Trying to detect target...({int(tmp+1)})")

				html = HTTPconnect(self.targetIP,self.proto,self.verbose,self.creds,False).Send(URI[tmp]).decode('utf-8')
				...
				for axis in range (0,len(html)):
					...
					if html[axis] == 'AXIS' or html[axis] == 'Axis' or html[axis] == 'axis':
						if not Vendor and not Product and html[axis+1] != ',':
							Vendor = 'AXIS'
							Product = html[axis+1]
							print(f"[i] Target found: {Vendor} {Product}")
							print(f"[i] Trying to detect firmware version...({int(tmp+1)})")
						...
						for axis in range (axis,len(html)):
							...
							if html[axis] == 'ver':
								...
								if html[axis+1].find('.') == True:
									Version = html[axis+1]
									print(f"[i] Version found: {Version}")
									break
					...
					# Version must have dots
						...
						if html[axis+1] == 'DVR':
							...
							Version = html[axis+4]
							print(f"[i] Target found: {Vendor} {Product} {Version}")
							continue
						else:
							continue
					...
					if Vendor and Product and Version:
						...
						target += " "
						target += Version
						print(f"[i] Verbose: {target.split()}")
						return
					continue
...
#
	...
	print(f"[i] Remote IP: {rhost}")
	print(f"[i] Remote PORT: {rport}")
